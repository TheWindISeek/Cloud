...
@SpringBootTest
class CloudComputationApplicationTests {

    @Test
    void contextLoads() {
        int cloudletId = 1;
        int cloudletLength = 10;//云任务长度
        int cloudletFileSize = 1;
        int cloudletOutputSize = 1;
        int pesNumber = 1;
        UtilizationModelFull full = new UtilizationModelFull();//100%始终百分百的分配资源

        //这样就创建了一个任务 不过目前我还是不知道full这个玩意到底是用来干嘛的
        Cloudlet cloudlet = new Cloudlet(cloudletId, cloudletLength,
                cloudletFileSize, cloudletOutputSize, pesNumber, full, full, full);
    }
